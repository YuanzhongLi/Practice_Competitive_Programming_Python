...
class Solution:
    def minJumps(self, arr: List[int]) -> int:
        N = len(arr)
        mp = {} # key: array value, value: [index]
        mp2 = {} # key: arr value, value: node id
        V = N
        ...
        for i in range(N):
            if i+1 < N:
                dijkstra.add_edge(i, i+1, 1)
            if i-1 >= 0:
                dijkstra.add_edge(i, i-1, 1)

        for key in mp.keys():
            root_id = mp2[key]
            for node_id in mp[key]:
                dijkstra.add_edge(node_id, root_id, 1)
                dijkstra.add_edge(root_id, node_id, 0)

        dijkstra.min_path(0)
        return dijkstra.dist[N-1]
