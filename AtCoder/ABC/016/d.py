...
from math import sqrt, atan2, sin, cos, radians, degrees, acos
...
def getCrossPoints(c, l):
    ...
    base = sqrt(c.r * c.r - normP(pr - c.c))
...
def getCrossPointsCC(c1, c2):
    ...
    a = acos((c1.r * c1.r + d * d - c2.r * c2.r) / (2 * c1.r * d)) #double
...
def rotate(p, baseP, theta):
    ...
    vec = p - baseP
    vecRotated = Point(vec.x * cos(rad_theta) - vec.y * sin(rad_theta), \
                       vec.x * sin(rad_theta) + vec.y * cos(rad_theta))
    return baseP + vecRotated

ax, ay, bx, by = map(float, input().rstrip().split())
